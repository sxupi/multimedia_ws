...
class RadioNode(Node):
    ...
    def __init__(self):
        ...
        # Subscribe to volume as Float32 (name suggests that)
        ...
        # Use our integrated driver
        self._radio = Si4703Radio(i2c_addr=0x10, reset_pin=23)

        self._curr_freq = 900    # 90.0 MHz initial
        self._curr_volume = 0.0

        # Set initial state on hardware
        try:
            self._radio.si4703SetChannel(self._curr_freq)
            self._radio.si4703SetVolume(
                self.__convert_volume(self._curr_volume))
        except OSError as e:
    ...
    def __volume_change_callback(self, msg: Float32) -> None:
        ...
        try:
            ...
            self.get_logger().error(f'I2C error setting volume: {e}')
    ...
    def __play_stop_handle(self) -> None:
        # TODO: Implement play/stop (e.g., mute / unmute)
        pass

    def __switch_handle(self) -> None:
        # TODO: Implement switching between sources, if needed
        pass
    ...
    def __find_next_channel(self) -> None:
        try:
            ...
            self.get_logger().error(f'I2C error during seek up: {e}')
    ...
    def __find_prev_channel(self) -> None:
        try:
            ...
            self.get_logger().error(f'I2C error during seek down: {e}')
...
def main(args=None):
    ...
    node = RadioNode()
    node.get_logger().info('Starting to spin the radio node now')
